...
//   const [sortBy, setSortBy] = useState("amount"); // New state to track sorting criteria
...
//       } else if (sortBy === "date") {
//         const dateA = new Date(a.date.split(',')[1].trim()); // Parse the date correctly
//         const dateB = new Date(b.date.split(',')[1].trim()); // Parse the date correctly
...
//   // Function to filter entries based on the tag
...
//     setSortOrder("asc");
//     setFilterTag("");
//     setSortBy("amount"); // Reset sorting criteria
//   };
...
import axios from "axios";
import Tesseract from "tesseract.js";
import { v4 as uuidv4 } from 'uuid'; // Import uuid to generate unique IDs for new entries
...
function UserDashboard() {
  ...
  const [editExpenseId, setEditExpenseId] = useState(null); // Changed from editIndex to editExpenseId
  const [showUploadPopup, setShowUploadPopup] = useState(false);
  const [sortOrder, setSortOrder] = useState("asc");
  ...
  const sortEntries = (entries) => {
    return [...entries].sort((a, b) => {
      if (sortBy === "amount") {
        ...
        // Date parsing needs to be robust. Assuming 'date' is like "Tue, 21/05/2024"
        // We'll parse it into a comparable Date object.
        const parseDateString = (dateStr) => {
          const parts = dateStr.split(',')[1].trim().split('/');
          // Date constructor: year, month-1, day
  ...
  const handleAdd = () => {
    ...
    setFormData({ tag: "", description: "", amount: "", date: currentDate, expense_id: uuidv4() }); // Generate new ID for new entry
    setEditExpenseId(null); // Ensure we're not in edit mode
    setShowForm(true);
  };
  ...
  const handleSave = async () => {
    ...
    const entryToSave = {
      ...
      date: formData.date || currentDate,
      email,
      timestamp: formData.timestamp || Date.now(), // Preserve timestamp if editing, generate if new
    };

    try {
      if (editExpenseId !== null) { // We are in edit mode, editExpenseId holds the ID of the entry being edited
        ...
        // After adding, it's best to refetch to ensure we get the latest data including the backend-assigned expense_id
        fetchEntries(email);
      }
  ...
  const handleEdit = (expenseId) => { // Accept expenseId directly
    const selected = entries.find(entry => entry.expense_id === expenseId); // Find by unique ID
    if (!selected) {
      console.error("Entry not found for editing:", expenseId);
      return;
    }

    // Ensure date is formatted correctly for the input type="date" if needed, or keep existing for display
    // For type="date" input, it expects 'YYYY-MM-DD' format.
    // Let's reformat the date from "Day, DD/MM/YYYY" to "YYYY-MM-DD" for the input field.
    const dateParts = selected.date.split(',')[1].trim().split('/');
    const formattedDateForInput = `${dateParts[2]}-${dateParts[1]}-${dateParts[0]}`;

    setFormData({ ...selected, date: formattedDateForInput }); // Populate with selected entry data
    setEditExpenseId(expenseId); // Set the ID of the entry being edited
  ...
  const handleDelete = async (expenseId) => { // Accept expenseId directly
    try {
      await axios.delete(`${API_ENDPOINT}?email=${sessionStorage.getItem("email")}&expense_id=${expenseId}`);
      const updatedEntries = entries.filter((entry) => entry.expense_id !== expenseId); // Filter by unique ID
      setEntries(updatedEntries);
    } catch (error) {
      console.error("Delete error:", error);
      alert("Failed to delete expense. Kindly reload the page!");
    ...
    reader.onload = async () => {
      try {
        ...
        const extractedData = {
          ...
          email: sessionStorage.getItem("email"),
          timestamp: Date.now(),
          expense_id: uuidv4(), // Generate a unique ID for the uploaded expense
        };

        await axios.post(API_ENDPOINT, extractedData);
        setEntries(prev => [...prev, extractedData]); // Add directly if no backend ID needed immediately, otherwise refetch
        setShowUploadPopup(false);
      } catch (err) {
        console.error("Tesseract Error:", err);
        alert("Failed to extract text. Try a clearer image.");
        ...
        <div className="flex flex-col items-center">
          ...
          <div className="overflow-x-auto w-full px-6">
            <table className="table-auto w-full border border-black text-center">
              ...
              <tbody>
                {displayedEntries.map((entry) => ( // No need for index here, use entry.expense_id for key
                  editExpenseId === entry.expense_id ? ( // Compare with unique ID
                    <tr key={entry.expense_id} className="bg-yellow-100">
                      ...
                      <td className="border px-2 py-1">
                        {/* The date input expects YYYY-MM-DD. formData.date is set to this in handleEdit */}
                        <input name="date" type="date" value={formData.date} onChange={handleChange} className="w-full p-1 border rounded" />
                      </td>
                      <td className="border px-2 py-1">
                        <button onClick={handleSave} className="text-green-600 font-semibold">Save</button>
                      </td>
                    ...
                    <tr key={entry.expense_id}> {/* Use unique ID as key */}
                      <td className="border px-4 py-2">{entry.tag}</td>
                      <td className="border px-4 py-2">{entry.description}</td>
                      <td className="border px-4 py-2">{entry.amount}</td>
                      <td className="border px-4 py-2">{entry.date}</td>
                      <td className="border px-4 py-2">
                        <button className="text-blue-500 underline" onClick={() => handleEdit(entry.expense_id)}>Edit</button> {/* Pass unique ID */}
                      </td>
                      <td className="border px-4 py-2">
                        <button className="text-red-500 underline" onClick={() => handleDelete(entry.expense_id)}>Delete</button> {/* Pass unique ID */}
                      </td>
                    </tr>
                  )
                ))}
                {showForm && editExpenseId === null && ( // Show add form only if not editing
                  <tr className="bg-green-100">
                    <td className="border px-2 py-1">
                      <input name="tag" value={formData.tag} onChange={handleChange} className="w-full p-1 border rounded" />
                    </td>
                    ...
                    <td className="border px-2 py-1">
                      {/* For new entries, formData.date might be pre-filled with current date but format needs adjustment for input type="date" */}
                      <input
                        name="date"
                        type="date"
                        value={formData.date.split(',')[1]?.trim().split('/').reverse().join('-') || ''} // Convert "Day, DD/MM/YYYY" to "YYYY-MM-DD" for input value
...
//         return sortOrder === "asc" ? amountA - amountB : amountB - amountA;
//       } else if (sortBy === "date") {
//         const dateA = new Date(a.date.split(',')[1].trim());
//         const dateB = new Date(b.date.split(',')[1].trim());
//         return sortOrder === "asc" ? dateA - dateB : dateB - dateA;
//       }
//       return 0;
//     });
...
//       weekday: 'short', year: 'numeric', month: 'short', day: 'numeric'
//     });
//     setFormData({ tag: "", description: "", amount: "", date: currentDate, expense_id: "" });
//     setEditIndex(null);
//     setShowForm(true);
//   };
